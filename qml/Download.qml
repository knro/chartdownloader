...
DownloadForm
{
    ...
    downloadButton.onClicked:
    {
        ...
        if (state == "idle")
        {
            var isSuccess = downloadManager.startDownload(airportID.text, isMinimumChecked, isAirportChecked, isSIDChecked, isStarChecked, isApproachChecked, isAerosoftChecked, qualityComboCurrentIndex);
            if (isSuccess)
            {
                state = "running"
            }
